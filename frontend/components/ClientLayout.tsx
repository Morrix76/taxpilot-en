...
const TrialBanner = ({ trialInfo }) => {
  ...
  return (
    <div className="mb-8 rounded-xl p-4 text-center text-sm text-white shadow-lg"
         style={{ 
...
export default function ClientLayout({ children }: ClientLayoutProps) {
  ...
  if (isPublicPage) {
    return (
      <div className="bg-gray-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200">
        {children}
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-surface text-slate-800 dark:text-slate-200 sidebar" 
         style={{ backgroundColor: 'var(--background-color)', color: 'var(--text-color)' }}>
      <aside className="w-64 shrink-0 border-r flex flex-col sidebar" 
             style={{ 
               backgroundColor: 'var(--surface-color)', 
               borderColor: 'color-mix(in srgb, var(--primary-color) 20%, transparent)' 
             }}>
        {/* Logo + toggle tema + Country Selector */}
        <div className="flex items-center justify-between h-24 px-4">
          <div className="flex items-center gap-3 select-none">
            <div className="h-11 w-11 rounded-3xl shadow-xl animate-[spin_10s_linear_infinite] flex items-center justify-center"
        ...
        <div className="px-4 py-4 border-t border-gray-200 dark:border-slate-700 space-y-3">
          {/* Country Selector */}
          <div className="flex justify-center">
            <CountrySelector />
          </div>
          
          {/* Studio Demo Dropdown */}
